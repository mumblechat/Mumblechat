...
import com.ramapay.app.entity.tokens.TokenFactory;
...
public class OpenSeaService
{
    ...
    private Request buildRequest(long networkId, String api)
    {
        ...
        if (!TextUtils.isEmpty(apiKey)
                && !apiKey.equals("...")
                && com.ramapay.app.repository.EthereumNetworkBase.hasRealValue(networkId)
        )
        {
    ...
    public void resetOffsetRead(List<Long> networkFilter)
    {
        ...
        {
            if (com.ramapay.app.repository.EthereumNetworkBase.hasOpenseaAPI(networkId))
            {
    ...
    {
        if (!com.ramapay.app.repository.EthereumNetworkBase.hasOpenseaAPI(token.tokenInfo.chainId))
        {
            return Single.fromCallable(() -> "");
        }
        else
