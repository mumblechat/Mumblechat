...
import static com.ramapay.app.entity.MagicLinkParcel.generateReverseTradeData;
import static com.ramapay.token.tools.ParseMagicLink.currencyLink;
import static com.ramapay.token.tools.ParseMagicLink.customizable;
import static com.ramapay.token.tools.ParseMagicLink.normal;
import static com.ramapay.token.tools.ParseMagicLink.spawnable;
import static com.ramapay.token.tools.ParseMagicLink.unassigned;
...
public class ImportTokenViewModel extends BaseViewModel implements TransactionSendHandlerInterface
{
    ...
    ImportTokenViewModel(GenericWalletInteract genericWalletInteract,
        ...
        this.ramaPayService = ramaPayService;
    ...
    public void importThroughFeemaster(String url)
    {
        try
        {
            initParser();
            MagicLinkData order = parser.parseUniversalLink(universalImportLink);
            disposable = ramaPayService.handleFeemasterImport(url, wallet.getValue(), network.getValue().chainId, order)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    {
        disposable = ramaPayService.checkFeemasterService(feemasterServer, network.getValue().chainId, importOrder.contractAddress)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
